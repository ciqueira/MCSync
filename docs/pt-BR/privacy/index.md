...
Versão do documento: `mcsync-privacy-2026-08-01`

Esta Política de Privacidade descreve como o MCSync ("MCSync", "o
...
Ao instalar ou usar o MCSync, você reconhece ter acesso a esta política.

## 1. Identificação do Controlador

O MCSync é desenvolvido e mantido por Magno Ciqueira, responsável pelo
...
## 2. O que o MCSync faz

O MCSync é um aplicativo nativo para macOS que conecta sua conta do Google
...
O MCSync conecta sua conta Google usando OAuth 2.0. Dependendo do escopo
...
O aplicativo utiliza o escopo que você selecionou explicitamente. Para alterar
o escopo, desconecte a conta e conecte novamente depois de escolher outro nível
de acesso.
...
O MCSync lê e grava arquivos nos volumes de armazenamento e nas pastas do
...
O MCSync mantém um histórico local de operações que registra:
...
O MCSync não inclui bibliotecas de analytics, relatórios de falhas,
...
você seleciona. O MCSync não lê Gmail, Google Agenda, Google Contatos nem
...
O MCSync incorpora o rclone ([rclone.org](https://rclone.org)) para
...
O MCSync não solicita acesso a contatos, microfone, câmera, localização ou
...
O MCSync não compartilha seus dados com nenhum terceiro, exceto:
...
Nenhum outro dado é compartilhado com qualquer parte. O MCSync não vende
nem aluga dados.

## 9. Segurança

O MCSync aplica as seguintes medidas para proteger seus dados:
...
O MCSync é um aplicativo profissional destinado a fluxos de trabalho de
...
Como o MCSync armazena todos os dados localmente no seu Mac, você pode
...
O MCSync é operado no Brasil. Buscamos cumprir os requisitos de proteção
